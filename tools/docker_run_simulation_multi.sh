...
WRAP="/tmp/docker_run_simulation_multi_${USER}.sh"
MON_TERM_PID=""
GUI_PID=""
...
if docker info >/dev/null 2>&1; then
  ...
  exec docker compose run --rm --no-deps radiation-headless python scripts/monitor/radiation_monitor_multi.py --host "${HOST}" --port "${PORT}"
else
  echo "[docker-run] docker access denied in monitor terminal; falling back to sudo"
  exec sudo docker compose run --rm --no-deps radiation-headless python scripts/monitor/radiation_monitor_multi.py --host "${HOST}" --port "${PORT}"
fi
EOF
chmod +x "${WRAP}"
...
echo "[docker-run] gui service=radiation-gui"
...
launch_monitor_terminal() {
  if [[ -n "${GNOME_TERM}" ]]; then
    "${GNOME_TERM}" --geometry="${COLS}x${ROWS}" --title="Radiation Monitor Multi" -- bash -ic '"$1"; rc=$?; echo; echo "[docker-run] monitor exited with code ${rc}"; exec bash' _ "${WRAP}" &
    MON_TERM_PID=$!
    return
  fi
...
docker_compose --profile gui up radiation-gui "$@" &
GUI_PID=$!
wait "${GUI_PID}"
