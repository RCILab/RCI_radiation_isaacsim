...
LAUNCH_PY="${WS_DIR}/scripts/run_simulation.py"
MONITOR_PY="${WS_DIR}/scripts/monitor/radiation_monitor_multi.py"
...
PIDFILE="/tmp/radiation_monitor_${USER}.pid"
WRAP="/tmp/radiation_monitor_${USER}.sh"
MON_TERM_PID=""
LAUNCH_PID=""
...
("${PY}" "${MONITOR_PY}" --host "${HOST}" --port "${PORT}") &
echo \$! > "${PIDFILE}"
wait
EOF
chmod +x "${WRAP}"
...
echo "[run] monitor udp://${HOST}:${PORT}  cols=${COLS} rows=${ROWS}"
